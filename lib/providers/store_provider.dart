...
class StoreProvider extends ChangeNotifier {
  ...
  static List<Map<String, dynamic>> _allStores = [
    {
      'id': '1',
      'name': 'GreenMart',
  ...
  static List<Map<String, dynamic>> get allStores => _allStores;

  // Get stores by owner
  static List<Map<String, dynamic>> getStoresByOwner(String ownerId) {
    return _allStores.where((store) => store['ownerId'] == ownerId).toList();
  }

  // Get active stores
  static List<Map<String, dynamic>> get activeStores {
    return _allStores.where((store) => store['status'] == 'Active').toList();
  }

  // Get total stores count
  static int get totalStores => _allStores.length;

  // Get active stores count
  static int get activeStoresCount => activeStores.length;
  ...
  // Update store
  static void updateStore(String storeId, Map<String, dynamic> updatedData) {
  ...
  // Delete store
  static void deleteStore(String storeId) {
    _allStores.removeWhere((store) => store['id'] == storeId);
  }

  // Toggle store status
  static void toggleStoreStatus(String storeId) {
    final index = _allStores.indexWhere((store) => store['id'] == storeId);
    if (index != -1) {
  ...
  // Get store by ID
  static Map<String, dynamic>? getStoreById(String storeId) {
    try {
  ...
  static List<Map<String, dynamic>> getStoresByCategory(String category) {
    return _allStores.where((store) => store['category'] == category).toList();
  }

  // Get stores by status
  static List<Map<String, dynamic>> getStoresByStatus(String status) {
    return _allStores.where((store) => store['status'] == status).toList();
  }

  // Search stores by name
  static List<Map<String, dynamic>> searchStores(String query) {
